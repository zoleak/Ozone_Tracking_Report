  ...
  test<-reactive({
    data%>%
      dplyr::filter(`Site Name` %in% input$site_name_select,
                    between(Date ,input$dates[1], input$dates[2]))%>%
      dplyr::group_by(`Site Name`)%>%
      dplyr::summarise(Average = round(mean(AQI_value,na.rm = T)),
                       Median = round(median(AQI_value,na.rm = T)),
                       Max = max(AQI_value,na.rm = T),
                       Min = min(AQI_value,na.rm = T))
      
    
  })
  ### Creates datatable under plot in daily max section ###
